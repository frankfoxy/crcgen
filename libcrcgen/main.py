...
def main():
    try:
        ...
        p = argparse.ArgumentParser(description="CRC algorithm HDL code generator (VHDL, Verilog, MyHDL)")
        ...
        g.add_argument("-p", "--python", action="store_true", help="Generate Python code (mainly useful for testing purposes)")
        g.add_argument("-c", "--c", action="store_true", help="Generate C code (mainly useful for testing purposes)")
        g.add_argument(
            "-T", "--polynomial-convert", metavar="POLYNOMIAL", type=str, help="Convert a polynomial from string to int or vice versa and then exit."
        )
        ...
        p.add_argument(
            ...
            "--algorithm",
            type=str,
            choices=CRC_PARAMETERS.keys(),
            default="CRC-32",
            help="Select the CRC algorithm. " "Individual algorithm parameters (e.g. polynomial) can be overridden with the options below.",
        )
        ...
        p.add_argument("-C", "--crc-in-param", type=str, default="c", help="Generated function/module crc input parameter name")
        p.add_argument("-o", "--crc-out-param", type=str, default="c_o", help="Generated module crc output parameter name")
        p.add_argument("-S", "--static", action="store_true", help="Generate static C function. (only if -c)")
        p.add_argument("-I", "--inline", action="store_true", help="Generate inline C function. (only if -c)")
        p.add_argument(
            "-O",
            "--optimize",
            type=argInt,
            default=CrcGen.OPT_ALL,
        ...
        if args.nr_crc_bits is not None and args.nr_crc_bits < 1:
        ...
        if args.polynomial_convert is not None:
            return poly_convert(args.polynomial_convert, args.nr_crc_bits, args.shift_right)
        ...
        if args.polynomial is not None:
            crcParameters["polynomial"] = poly2int(args.polynomial, crcParameters["nrBits"], crcParameters["shiftRight"])
        ...
        if args.test:
            ...
            func_name = "%s%d_d%d" % (args.name, args.nr_crc_bits, args.nr_data_bits)
            if args.python:
                print(gen.genPython(funcName=func_name, crcVarName=args.crc_in_param, dataVarName=args.data_param))
            elif args.verilog_function:
                print(gen.genVerilog(genFunction=True, name=func_name, inDataName=args.data_param, inCrcName=args.crc_in_param, outCrcName=args.crc_out_param))
            elif args.verilog_module:
                print(gen.genVerilog(genFunction=False, name=func_name, inDataName=args.data_param, inCrcName=args.crc_in_param, outCrcName=args.crc_out_param))
            elif args.vhdl:
                print(gen.genVHDL(name=func_name, inDataName=args.data_param, inCrcName=args.crc_in_param, outCrcName=args.crc_out_param))
            elif args.myhdl:
                print(gen.genMyHDL(blockName=func_name, inDataName=args.data_param, inCrcName=args.crc_in_param, outCrcName=args.crc_out_param))
            elif args.c:
                print(gen.genC(funcName=func_name, crcVarName=args.crc_in_param, dataVarName=args.data_param, static=args.static, inline=args.inline))
            else:
                assert False
        return 0
